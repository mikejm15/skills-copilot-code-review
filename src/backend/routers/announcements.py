...
def list_announcements():
    now = datetime.utcnow()
    anns = list(announcements_collection.find({"expiration_date": {"$gte": now}}))
    for ann in anns:
        ann["_id"] = str(ann["_id"])
    return anns
...
def update_announcement(announcement_id: str, data: AnnouncementCreate, user=Depends(get_current_user)):
    result = announcements_collection.update_one({"_id": announcement_id}, {"$set": data.dict()})
